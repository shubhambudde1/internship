...
const ReturnReq = () => {
  ...
  const fetchReturns = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get('http://localhost:5001/api/ordeReturn'); // Use your backend URL
  ...
  const handleApprove = async (id) => {
    try {
      await axios.put(`http://localhost:5001/api/ordeReturn/${id}/approve`);
      ...
      toast.error(`Failed to approve request #${id}.`);
    }
  };
  ...
  const handleReject = async (id) => {
    try {
      await axios.put(`http://localhost:5001/api/ordeReturn/${id}/reject`);
      setReturns((prevReturns) =>
        prevReturns.map((req) =>
      ...
      toast.error(`Failed to reject request #${id}.`);
    }
  };
  ...
  return (
    <div className="container mx-auto p-4 md:p-6">
      ...
      {error && (
        <p className="text-center text-red-600 bg-red-100 p-3 rounded">{error}</p>
      )}
